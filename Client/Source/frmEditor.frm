...
If Dir(App.Path & "\Scripts\" & AFileName) <> "" Then
'If Dir(App.Path & "\Scripts\Main.ess") <> "" Then
    hFile = FreeFile
    Open App.Path & "\Scripts\" & AFileName For Input As #hFile
    'Open App.Path & "\Scripts\Main.ess" For Input As #hFile
...
Private Sub mnuSave_Click()
    Open App.Path & "\Scripts\" & AFileName For Output As #1
    'Open App.Path & "\Scripts\Main.ess" For Output As #1
        Print #1, RT.Text
    Close #1
    
    ' If you are making this editor edit more than scripts...
    ' Find a way to change this!
    Call SendData("NEWMAIN" & SEP_CHAR & AFileName & SEP_CHAR & RT.Text & SEP_CHAR & END_CHAR)
